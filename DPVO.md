...
We provide a CUDA-ready Docker setup under `docker/`.

```bash
# build the image (requires NVIDIA Container Toolkit)
docker build -t dpvo-runtime docker/

# process a video via the helper script
./run.sh /absolute/path/to/video.mp4 [--name my_run]
```

The script mounts the host `outputs/` directory into the container so each run writes results to `outputs/<run_name>` on the host without entering the container. Set `DPVO_IMAGE_NAME` or `DPVO_OUTPUT_DIR` to override the defaults.
